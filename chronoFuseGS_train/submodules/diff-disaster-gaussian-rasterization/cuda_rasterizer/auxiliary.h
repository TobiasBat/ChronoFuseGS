...
__forceinline__ __device__ bool in_viewport(int idx,
	const float* means,
	const float* viewmatrix,
	const float* projmatrix,
	float3& p_view)
{
	float3 p_orig = { means[3 * idx], means[3 * idx + 1], means[3 * idx + 2] };
	...
	if (p_view.z <= 0.2f || p_proj.x < -1.0 || p_proj.x > 1.0 || p_proj.y < -1.0 || p_proj.y > 1.0) // really 1.3 better pick 1
	{
		return false;
	}
	return true;
