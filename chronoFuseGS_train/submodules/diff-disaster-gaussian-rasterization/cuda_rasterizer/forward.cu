...
__global__ void preprocessCUDA(
    ...
    const float* opacities, const float* shs, const float* opacityActivations,
    const float* colorActivations, bool* clamped, const float* cov3D_precomp,
    ...
    const int t, const int numT, int* radii, float2* points_xy_image,
  ...
  if (colors_precomp == nullptr) {
    ...
    rgb[idx * C + 0] *= colorActivations[idx * 3 * numT + t * 3 + 0];
    rgb[idx * C + 1] *= colorActivations[idx * 3 * numT + t * 3 + 1];
    rgb[idx * C + 2] *= colorActivations[idx * 3 * numT + t * 3 + 2];
  } else {
    // colors_precomp[idx * C + 0] *= colorActivations[idx * 3 * numT + t * 3 +
    // 0]; colors_precomp[idx * C + 1] *= colorActivations[idx * 3 * numT + t *
    // 3 + 1]; colors_precomp[idx * C + 2] *= colorActivations[idx * 3 * numT +
    // t * 3 + 2];
  ...
  // TODO add color activation here
  // rgb[idx * C + 0] *= colorActivations[idx * 3 * numT + t * 3 + 0];
  // rgb[idx * C + 1] *= colorActivations[idx * 3 * numT + t * 3 + 1];
  // rgb[idx * C + 2] *= colorActivations[idx * 3 * numT + t * 3 + 2];
  ...
  float opacity = opacities[idx] * opacityActivations[idx * numT + t];
...
__global__ void __launch_bounds__(BLOCK_X* BLOCK_Y)
    renderCUDA(const uint2* __restrict__ ranges,
               const uint32_t* __restrict__ point_list, int W, int H, int t,
               int numT, const float2* __restrict__ points_xy_image,
               const float* __restrict__ features,
               const float4* __restrict__ conic_opacity,
               const float* __restrict__ colorActivations,
               float* __restrict__ final_T, uint32_t* __restrict__ n_contrib,
               const float* __restrict__ bg_color,
               float* __restrict__ out_color, const float* __restrict__ depths,
               float* __restrict__ invdepth,
               int* __restrict__ out_gaus_rendered, bool validate) {
  ...
  for (int i = 0; i < rounds; i++, toDo -= BLOCK_SIZE) {
    ...
    for (int j = 0; !done && j < min(BLOCK_SIZE, toDo); j++) {
      ...
      for (int ch = 0; ch < CHANNELS; ch++) {
        ...
        // C[ch] += features[collected_id[j] * CHANNELS + ch] * alpha * T *
        // colorActivations[collected_id[j] * CHANNELS * numT + t * CHANNELS +
        // ch];
      }
      if (invdepth)
      ...
      // TODO whats the thresshold here?
      // should this be really an atomic action
      if (validate && alpha * T >= 0.01f)
        atomicExch(&out_gaus_rendered[collected_id[j]], 1);
    }
  }
...
void FORWARD::render(const dim3 grid, dim3 block, const uint2* ranges,
                     const uint32_t* point_list, int W, int H, int t, int numT,
                     const float2* means2D, const float* colors,
                     const float4* conic_opacity, const float* colorActivations,
                     float* final_T, uint32_t* n_contrib, const float* bg_color,
                     float* out_color, float* depths, float* depth,
                     int* out_gaus_rendered, bool validate) {
  renderCUDA<NUM_CHANNELS><<<grid, block>>>(
      ranges, point_list, W, H, t, numT, means2D, colors, conic_opacity,
      colorActivations, final_T, n_contrib, bg_color, out_color, depths, depth,
      out_gaus_rendered, validate);
}
...
void FORWARD::preprocess(
    ...
    const int t, const int numT, int* radii, float2* means2D, float* depths,
    float* cov3Ds, float* rgb, float4* conic_opacity, const dim3 grid,
    uint32_t* tiles_touched, bool prefiltered, bool antialiasing) {
  preprocessCUDA<NUM_CHANNELS><<<(P + 255) / 256, 256>>>(
      P, D, M, means3D, scales, scale_modifier, rotations, opacities, shs,
      opacityActivations, colorActivations, clamped, cov3D_precomp,
      colors_precomp, viewmatrix, projmatrix, cam_pos, W, H, tan_fovx, tan_fovy,
      focal_x, focal_y, t, numT, radii, means2D, depths, cov3Ds, rgb,
      conic_opacity, grid, tiles_touched, prefiltered, antialiasing);
}
