...
__device__ void computeColorFromSH(int idx, int deg, int max_coeffs, const glm::vec3* means, glm::vec3 campos, const float* shs, const bool* clamped, const glm::vec3* dL_dcolor, glm::vec3* dL_dmeans, glm::vec3* dL_dshs)
{
	...
	dL_dRGB.x *= clamped[3 * idx + 0] ? 0 : dL_dRGB.x;
	dL_dRGB.y *= clamped[3 * idx + 1] ? 0 : dL_dRGB.y;
	dL_dRGB.z *= clamped[3 * idx + 2] ? 0 : dL_dRGB.z;
...
__global__ void preprocessCUDA(
	...
	glm::vec4* dL_drot,
	float* dL_dopacity,
	const float* opacity_acivations,
	const float* color_activations,
	float* dL_opacity_activations,
	float* dL_color_activations,
	const int t,
	const int numT)
{
	auto idx = cg::this_grid().thread_rank();
	...
	// function_f(o) * function_g(a) -> d/da = f(o) * g'(a)
	// function_f(o) * function_g(a) -> d/do = g(a) * f'(o)
	dL_opacity_activations[idx * numT + t] = opacities[idx] * dL_dopacity[idx]; // TODO  check if this is realy right and not just g'(a) which is I guess 1
	dL_dopacity[idx] = opacity_acivations[idx * numT + t] * dL_dopacity[idx]; 


	// TODO do color activation gradient stuff
	dL_color_activations[idx * 3 * numT + t * 3 + 0] = colors[idx * C + 0] * dL_dcolor[idx * C + 0]; // clamped[idx] ? 0.0 : dL_dcolor[idx * C + 0]; // clamped[idx] ? 0.0 : 1.0;
	dL_color_activations[idx * 3 * numT + t * 3 + 1] = colors[idx * C + 1] * dL_dcolor[idx * C + 1]; // clamped[idx] ? 0.0 : dL_dcolor[idx * C + 1]; // clamped[idx] ? 0.0 : 1.0;
	dL_color_activations[idx * 3 * numT + t * 3 + 2] = colors[idx * C + 2] * dL_dcolor[idx * C + 2]; // clamped[idx] ? 0.0 : dL_dcolor[idx * C + 2]; // clamped[idx] ? 0.0 : 1.0;

	// TODO 
	dL_dcolor[idx * C + 0] *= color_activations[idx * 3 * numT + t * 3 + 0]; 
	dL_dcolor[idx * C + 1] *= color_activations[idx * 3 * numT + t * 3 + 1]; 
	dL_dcolor[idx * C + 2] *= color_activations[idx * 3 * numT + t * 3 + 2]; 
...
renderCUDA(
	...
	const float* __restrict__ color_activations,
	...
	float* __restrict__ dL_color_activations
)
{
	...
	for (int i = 0; i < rounds; i++, toDo -= BLOCK_SIZE)
	{
		...
		for (int j = 0; !done && j < min(BLOCK_SIZE, toDo); j++)
		{
			...
			for (int ch = 0; ch < C; ch++)
			{
				...
				// atomicAdd(&(dL_dcolors[global_id * C + ch]), dchannel_dcolor * dL_dchannel * color_activations[global_id * C * numT + t * C + ch]);
				// global id for dl and collected id for color? 
				// atomicAdd(&(dL_color_activations[global_id * C * numT + t * C + ch]), dchannel_dcolor * dL_dchannel * colors[collected_id[j] * C + ch]);
				// atomicAdd(&(dL_color_activations[global_id * C * numT + t * C + ch]), dchannel_dcolor * dL_dchannel * colors[global_id * C + ch]);
			...
			atomicAdd(&(dL_dopacity[global_id]), G * dL_dalpha); // TODO here probably we have to add the activation 
		}
	}
}
...
void BACKWARD::preprocess(
	...
	const bool* clamped,
	const float* opacities,
	const float* opacity_activations,
	const float* color_activaitons,
	const int t,
	const int numT,
	const glm::vec3* scales,
	const glm::vec4* rotations,
	...
	glm::vec3* dL_dscale,
	glm::vec4* dL_drot,
	float* dL_dopacity_activations,
	float* dL_dcolor_activations,
	bool antialiasing)
{
	...
	preprocessCUDA<NUM_CHANNELS> << < (P + 255) / 256, 256 >> > (
		...
		opacity_activations,
		color_activaitons,
		dL_dopacity_activations,
		dL_dcolor_activations,
		t,
		numT);
}
